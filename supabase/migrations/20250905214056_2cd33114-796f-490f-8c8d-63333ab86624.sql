-- Update the existing profile for the new user with better data
UPDATE profiles 
SET 
  name = 'Sarah Chen',
...
-- Add some likes and views from the original test user to the new user's resources (only if resources exist)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM resources WHERE author_id = '2bdcce68-b238-4915-bdd8-bcd3133964d6') THEN
